...
Electron first probes `http://127.0.0.1:9000/`. If no service is already running, it starts `server.py` with the project `.venv` Python executable and loads the dedicated `overlay.html` renderer. The shell is a single always-on-top transparent, frameless window with a compact capsule and directly connected expanded panel; drag the header to move it.
