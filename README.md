...
使用云端或带鉴权的兼容端点时，请修改 `config/model_settings.json` 中对应 Profile 的 `base_url`、`model`，并在 `.env` 填写该 Profile 的 `api_key_env`。

## 文本模型配置

非敏感配置位于 [`config/model_settings.json`](config/model_settings.json)，密钥位于本机 `.env`。浏览器中的模型配置弹窗已经移除。
...
1. 修改 `config/model_settings.json` 的 `active_profile`。
...
| `/` | 前端网页和静态资源 |
| `/api/chat/` | SSE 流式文本回答 |
| `/api/model-config/` | 不含密钥和地址的活动模型摘要 |
| `/api/prompt/` | 系统提示词 |
...
```text
server.py                         统一 FastAPI、ASR WebSocket 和静态网页入口
llm_api.py                        文本模型 HTTP/SSE API
llm_providers.py                  OpenAI 与 Anthropic 流式协议适配
config/model_profiles.py          配置验证、活动 profile 和密钥解析
config/model_settings.json        非敏感模型预设
local_asr.py                      本地模型加载、流式识别和断句
static/scripts.js                 同源 WebSocket、问题状态和 LLM 交互
start.bat                         Windows 一键启动入口
```

本地化的是实时语音识别；配置的文本模型仍可能是远程服务。当前版本尚未进行 EXE 或 APK 打包。

## Windows 桌面隐私模式

Windows 桌面版位于 [`desktop/`](desktop/)，它会启动或复用现有 Python 服务并加载同一套前端。开发启动方式：
...
Electron 窗口默认启用系统级捕获保护，并使用透明、无边框、置顶的 Cluely 风格专用悬浮渲染器；桌面端加载 `static/overlay.html`，不再加载旧网页工作台。收起时是紧凑胶囊，展开时胶囊与回复面板直接连接。底层调用 `BrowserWindow.setContentProtection(true/false)`，由 Electron 映射到当前平台支持的窗口内容保护机制。点击胶囊中的保护按钮或按 `Ctrl+Shift+P` 可以切换保护状态，状态徽标会显示保护已开启、已关闭、系统不支持或保护失败。
